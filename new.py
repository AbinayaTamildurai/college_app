...
def ensure_columns(df):
    ...
    meta_cols = [
        ...
        "Father's Name", "Address", "Mobile Number", "Aadhar Number", "First Graduate", "Collected By"
    ]
    for c in meta_cols:
        if c not in df.columns:
            df[c] = ""

    if "Collected By" not in df.columns:
       df["Collected By"] = ""
...
def load_credentials():
    if os.path.exists(CREDENTIALS_FILE):
        ...
        return creds_df.set_index("username").to_dict(orient="index")
    return {}

def login_page():
    display_logo()
    st.title("🔐 Login Portal")
    st.subheader("Dr.G.U.Pope College Of Engineering")

    valid_users = load_credentials()

    role = st.selectbox("Select Role", ["Admin", "Staff", "Student"], key="login_role")
    ...
    if st.button("Login", key="login_button"):
        if username in valid_users:
            ...
            if user_info["password"] == password and user_info["role"] == role:
                ...
                st.error("❌ Invalid username/password or wrong role selected")
        else:
...
def search_student_page(df):
    ...
    st.markdown("### 👤 Last Payment Collected By")
    st.info(student_info.get("Collected By", "Not Available"))
...
def pay_fees_page(df):
    ...
    if not student.empty:
        ...
        if st.button("Submit Payment"):
            idx = student.index[0]
            # Save who collected the fees
            df.at[idx, "Collected By"] = st.session_state.get("admin_user", "Unknown")
            ...
            receipt_html = f"""
              <div style="border:2px dashed #4CAF50; padding:25px; font-family:Arial; background:#f9f9f9;">
                ...
                <div style="background:#e8f5e9; padding:10px; border-radius:5px; margin-bottom:15px;">
                  ...
                  <p><strong>Collected By:</strong> {st.session_state.get('admin_user','')}</p>
                </div>
